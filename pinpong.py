...
# Pro uložení výsledku zápasu do databáze
...
def zadat_jmeno(player_num):
    ...
    while True:
        ...
        screen.blit(prompt_text, (WIDTH // 2 - prompt_text.get_width() // 2, HEIGHT // 2 - 50)) # Zobrazení promptu na střed obrazovky
        ...
        # Ukončení hry při zavření okna
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                return
            
            # Mazání textu, potvrzení zadání jména a pokračování do hry
...
def hra():
    ...
    while running:
        ...
        if ball_x <= paddle_width and player1_y <= ball_y <= player1_y + paddle_height:
            ...
            ball_dy += relative_intersect_y * 5  # Vertikálního směr
        ...
        if ball_x >= WIDTH - paddle_width - ball_size and player2_y <= ball_y <= player2_y + paddle_height:
            ...
            ball_dy += relative_intersect_y * 5  # Vertikálního směru
...
def menu():
    while True:
        ...
        # Vykreslení nadpisu obrazovky a tlačítek, zarovnaných na střed
...
# Pro zobrazení historie výsledků
def zobrazit_historii_vysledku():
    while True:
        ...
        # Zobrazení výsledků zápasů na obrazovce s vertikálním posunem
        y_offset = 100
        ...
        # Zpracování kliknutí na tlačítko "Zpět" a zavření okna
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                return
